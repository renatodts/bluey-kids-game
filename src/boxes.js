...
function createBasket() {
  ...
  const side = new THREE.Mesh(
    ...
    lambert('#c98d4e', { side: THREE.DoubleSide })
  );
  side.position.y = 0.45;
  const bottom = new THREE.Mesh(new THREE.CircleGeometry(0.8, 18), lambert('#b57c40'));
  bottom.rotation.x = -Math.PI / 2;
  bottom.position.y = 0.02;
  const rim = new THREE.Mesh(new THREE.TorusGeometry(1.0, 0.07, 8, 24), lambert('#a86f38'));
...
function createChest() {
  const group = new THREE.Group();
  const body = new THREE.Mesh(new THREE.BoxGeometry(2.0, 0.9, 1.3), lambert('#8a5a33'));
...
function createBed() {
  const group = new THREE.Group();
  const base = new THREE.Mesh(new THREE.BoxGeometry(2.2, 0.4, 1.4), lambert('#b0623c'));
  base.position.y = 0.2;
  const mattress = new THREE.Mesh(new THREE.BoxGeometry(2.05, 0.25, 1.25), lambert('#f2f2f2'));
  mattress.position.y = 0.5;
  const pillow = new THREE.Mesh(new THREE.BoxGeometry(0.6, 0.2, 1.0), lambert('#ffd166'));
  pillow.position.set(-0.65, 0.7, 0);
  const headboard = new THREE.Mesh(new THREE.BoxGeometry(0.12, 1.0, 1.4), lambert('#b0623c'));
