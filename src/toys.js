...
function createBall(color) {
  ...
  const sphere = new THREE.Mesh(new THREE.SphereGeometry(0.45, 20, 14), lambert(color));
  sphere.position.y = 0.45;
  const stripe = new THREE.Mesh(new THREE.TorusGeometry(0.45, 0.06, 10, 24), lambert('#ffffff'));
...
function createBlock(color) {
  const group = new THREE.Group();
  const base = new THREE.Mesh(new THREE.BoxGeometry(0.7, 0.5, 0.5), lambert(color));
  base.position.y = 0.25;
  group.add(base);
  for (const dx of [-0.175, 0.175]) {
    for (const dz of [-0.125, 0.125]) {
      const stud = new THREE.Mesh(new THREE.CylinderGeometry(0.09, 0.09, 0.12, 12), lambert(color));
...
function createPlush(color) {
  const group = new THREE.Group();
  const body = new THREE.Mesh(new THREE.SphereGeometry(0.35, 16, 12), lambert(color));
  body.position.y = 0.32;
  body.scale.y = 0.95;
  const head = new THREE.Mesh(new THREE.SphereGeometry(0.25, 16, 12), lambert(color));
  head.position.y = 0.78;
  const snout = new THREE.Mesh(new THREE.SphereGeometry(0.1, 10, 8), lambert('#f5e6c8'));
  snout.position.set(0, 0.72, 0.2);
  const earMaterial = lambert(color);
